<x-app-layout>
    @section('title', 'Prioridades Eliminadas')
    <x-slot name="header">
        <div>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Prioridades Eliminadas') }}
            </h2>
        </div>
    </x-slot>
    <div class="content">
                                    ...
                                    @foreach ($trashed as $prioridad)
                                        <tr
                                            ...
                                            <td
                                                ...
                                                <div class="text-sm">
                                                    <div class="font-medium text-gray-700 uppercase">{{$prioridad->clase->name}} - {{$prioridad->name}}</div>
                                                </div>
                                            </td>
                                            ...
                                            <td
                                                ...
                                                <div class="grid grid-cols-1">
                                                    <div>
                                                        {{-- restaurar --}}
                                                        <form method="POST" action="{{ route('prioridad_restore') }}"
                                                            onsubmit="return confirm('Restaurar Prioridad');">
                                                            {{ csrf_field() }}
                                                            <input type="hidden" name="id"
                                                                value="{{ $prioridad->id }}" required />
                                                            <button type="submit">
                                    ...
                                    <tr>
                                        <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b  block lg:table-cell relative lg:static"
                                            ...
                                            <span
                                                ...
                                                registros</span>
                                            {{ __('No hay prioridades eliminadas') }}
                                        </td>
                                    </tr>
                                @endif
                            </tbody>
