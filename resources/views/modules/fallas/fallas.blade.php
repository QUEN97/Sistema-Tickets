            ...
            <form action="{{ route('fallas') }}" method="GET">
                <div class="flex">
                    <div class="relative mr-4">
                        <label for="filter" class="sr-only">Filtrar por servicio</label>
                        <select name="filter" id="filter"
                            ...
                            @foreach ($servicios as $servicio)
                                <option value="{{ $servicio->id }}"
                                    {{ request('filter') == $servicio->id ? 'selected' : '' }}>{{ $servicio->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <tr
                            ...
                            <th
                                ...
                                <div class="w-full flex justify-center gap-2">
                                    ...
                                    {{ $falla->created_at->locale('es')->isoFormat('D / MMMM / YYYY H:mm:ss a') }}
                                </div>
                            </th>
                            <th
                                class="w-full font-medium text-sm lg:w-auto p-3 text-gray-800 text-center border border-b dark:text-gray-400  dark:border-gray-700">
