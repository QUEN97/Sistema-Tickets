<div>
    <div class="py-4 space-y-4">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            {{-- Barra de Busqueda --}}
            <div>
                <x-input wire:model="search" type="text" class="w-auto" placeholder="Buscar fallas..." />
            </div>
            {{-- Acciones Masivas --}}
            @if ($checked)
                <x-dropdown align="right" width="60">
                    ...
                    <x-slot name="content">
                        <div class="w-60">
                            ...
                            <div>
                                @if ($valid->pivot->de == 1)
                                    <x-dropdown-link href="#" wire:click="deleteFallas">
                                        {{ __('Eliminar Falla') }}
                                    </x-dropdown-link>
                                @endif
        ...
        @if ($selectPage)
            @if ($selectAll)
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ $fallas->total() }}</strong> elementos.
                </div>
            @else
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ count($checked) }}</strong> elementos, ¿Deseas seleccionar los
                    <strong>{{ $fallas->total() }}</strong>?
        ...
        <div class="overflow-auto rounded-lg shadow hidden md:block">
            ...
            <x-table>
                <x-slot name="head">
                    ...
                    <x-heading sortable wire:click="sortBy('name')" :direction="$sortField === 'name' ? $sortDirection : null">FALLA</x-heading>
                    <x-heading sortable wire:click="sortBy('servicio_id')" :direction="$sortField === 'servicio_id' ? $sortDirection : null">SERVICIO</x-heading>
                ...
                <x-slot name="body">
                    @forelse($fallas as $falla)
                        ...
                        <x-row wire:loading.class.delay="opacity-75">
                            {{-- Componente Column --}}
                            <x-cell> <x-input type="checkbox" value="{{ $falla->id }}" wire:model="checked" />
                            </x-cell>
                            <x-cell>{{ $falla->id }} </x-cell>
                            <x-cell class="font-bold">{{ $falla->name }}</x-cell>
                            <x-cell>{{ $falla->servicio->name }}</x-cell>
                            <x-cell>
                                <span
                                    class="rounded bg-{{ $falla->status_color }}-200 py-1 px-3 text-xs text-{{ $falla->status_color }}-500 font-bold">
                                    {{ $falla->status }}
                                </span>
                            </x-cell>
                            <x-cell> {{ $falla->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                            </x-cell>
                            <x-cell>
                                <div class="flex gap-2 justify-center items-center">
                                    @if ($valid->pivot->ed == 1)
                                        @livewire('fallas.edit-falla', ['fallaID' => $falla->id], key('ed'.$falla->id))
                                    @endif
                                    @if ($valid->pivot->de == 1)
                                        @livewire('fallas.delete-falla', ['fallaID' => $falla->id], key('del'.$falla->id))
                                    @endif
                                </div>
                            </x-cell>
                        </x-row>
            ...
            @forelse($fallas as $falla)
                <div class="bg-white dark:bg-slate-900 space-y-3 p-4 rounded-lg shadow">
                        ...
                        <div x-data="{ open: false }">
                            <div class="relative inline-block text-left">
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    <div class="flex gap-2 justify-center items-center">
                                        @if ($valid->pivot->ed == 1)
                                        @livewire('fallas.edit-falla', ['fallaID' => $falla->id], key('edit'.$falla->id))
                                    @endif
                                    @if ($valid->pivot->de == 1)
                                        @livewire('fallas.delete-falla', ['fallaID' => $falla->id], key('delete'.$falla->id))
                                    @endif
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="flex items-center space-x-2 text-sm">
                        <div class="font-bold">#{{ $falla->id }}</div>
                    ...
                    <div class="flex gap-4 justify-center items-center md:flex-row md:items-center md:justify-center ">
                        ...
                        <div>
                            {{ $falla->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                        </div>
                    </div>
                </div>
            @empty
