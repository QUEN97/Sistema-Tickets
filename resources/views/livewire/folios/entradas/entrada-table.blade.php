                ...
                <x-slot name="body">
                    @forelse($folios as $folio)
                        ...
                        <x-row wire:loading.class.delay="opacity-75">
                            {{-- Componente Column --}}
                            <x-cell class="font-bold">{{ $folio->folio }} </x-cell>
                            <x-cell>
            ...
            @forelse($folios as $folio)
                <div class="bg-white dark:bg-slate-900 space-y-3 p-4 rounded-lg shadow">
                        ...
                        <div x-data="{ open: false }">
                            <div class="relative inline-block text-left">
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    tabindex="-1">
                                    <div class="flex gap-2 justify-center items-center">
                                        @livewire('folios.entradas.show-folio',['folioID'=>$folio->id], key('showfolio'.$folio->id)) 
                                        @livewire('folios.entradas.entrada-export',['folioID'=>$folio->id], key('downfolio'.$folio->id))
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex gap-4 justify-center items-center md:flex-row md:items-center md:justify-center ">
                        ...
                        <div>
                            {{ $folio->updated_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                        </div>
                    </div>
                </div>
            @empty
