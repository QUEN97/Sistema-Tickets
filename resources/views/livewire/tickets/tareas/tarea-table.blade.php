<div>
    <div class="py-4 space-y-4">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            {{-- Barra de Busqueda --}}
            <div>
                <x-input wire:model="search" type="text" class="w-auto" placeholder="Buscar tareas..." />
            </div>
            {{-- Acciones Masivas --}}
            @if ($checked)
                <x-dropdown align="right" width="60">
                    ...
                    <x-slot name="content">
                        <div class="w-60">
                            ...
                            <div>
                                {{-- @if ($valid->pivot->de == 1)
                                    <x-dropdown-link href="#" wire:click="deleteTareas">
                                        {{ __('Eliminar Tarea') }}
                                    </x-dropdown-link>
                                @endif --}}
        ...
        @if ($selectPage)
            @if ($selectAll)
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ $tareas->total() }}</strong> elementos.
                </div>
            @else
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ count($checked) }}</strong> elementos, ¿Deseas seleccionar los
                    <strong>{{ $tareas->total() }}</strong>?
                ...
                <x-slot name="body">
                    @forelse($tareas as $tarea)
                        ...
                        <x-row wire:loading.class.delay="opacity-75">
                            ...
                            <x-cell class="font-bold">#{{ $tarea->id }} </x-cell>
                            <x-cell class="font-bold">#{{ $tarea->ticket->id }}</x-cell>
                            ...
                            <x-cell> {{ $tarea->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                            </x-cell>
                            <x-cell> {{ $tarea->updated_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                            </x-cell>
                            <x-cell>
                                <div class="flex gap-2 justify-center items-center">
                                    ...
                                    @if ($tarea->status != 'Cerrado' || Auth::user()->permiso_id == 1)
                                        @if (Auth::user()->permiso_id == 1)
                                            ...
                                            @livewire('tickets.tareas.delete-tarea', ['tareaID' => $tarea->id], key('del' . $tarea->id))
                                        @endif
                                    @endif
                                </div>
                            </x-cell>
            ...
            @forelse($tareas as $tarea)
                <div class="bg-white dark:bg-slate-800 space-y-3 p-4 rounded-lg shadow">
                        ...
                        <div x-data="{ open: false }">
                            <div class="relative inline-block text-left">
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    tabindex="-1">
                                    <div class="flex gap-2 justify-center items-center">
                                        @livewire('tickets.tareas.show-tarea', ['tareaID' => $tarea->id], key('showtarea' . $tarea->id))

                                        @if ($tarea->status != 'Cerrado' || Auth::user()->permiso_id == 1)
                                            @if (Auth::user()->permiso_id == 1)
                                                <div>
                                                    <a href="{{ route('tck.tarea', $tarea->ticket_id) }}"
                                                        class="tooltip" :key="'edittarea-'.$tarea->id">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                    ...
                    <div class="flex items-center space-x-2 text-sm">
                        <div class="font-bold">TAREA. #{{ $tarea->id }}</div>
                    ...
                    <div class="flex gap-4 justify-center items-center md:flex-row md:items-center md:justify-center ">
                        ...
                        <div>
                            {{ $tarea->updated_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                        </div>
                    </div>
                </div>
            @empty
