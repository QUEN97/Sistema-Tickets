<div>
    <button wire:click="ConfirmDelete({{$tareaID}})" wire:loading.attr="disabled" class="tooltip">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-black hover:text-red-500">
    ...
    <x-dialog-modal wire:model="modalDelete" class="flex items-center">
        ...
        <x-slot name="content" class="relative">
            <div>
                ...
                <span class="dark:text-white">¿Está seguro de eliminar la tarea: <i>"{{$tName}}"</i> del Ticket: <i>{{$ticketID}}</i> ?</span>
            </div>
        </x-slot>
        <x-slot name="footer" >
