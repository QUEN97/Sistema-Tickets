    ...
    <div class="flex flex-wrap gap-2 justify-evenly">
        @forelse ($tickets as $tck)
            <div :class="$wire.c || 'w-full flex  flex-wrap-reverse md:flex-nowrap gap-2 justify-end items-center'"
                ...
                <div
                    :class="$wire.c ? 'flex justify-between gap-1 flex-col pb-3' :
                        'w-full px-1 grid max-sm:grid-cols-1 max-md:grid-cols-3 md:grid-cols-6 items-center gap-1'">
                ...
                <div class="relative" :class="$wire.c ? 'h-8 overflow-hidden' : ''">
                    <div class="flex flex-wrap justify-center items-center px-2" x-data="{
                        menu: false,
                        drop: false,
                        init() {
                            $wire.c ?
                        ...
                        <div :class="menu
                            ...
                            @if ($tck->status != 'Cerrado')
                                ...
                                @endif
                                <a :class="menu ? 'flex gap-1' : 'tooltip'" href="{{ route('tck.ver', $tck->id) }}">
                                ...
                                @endif
                            @else
                                <div :class="menu && 'flex gap-1 w-max'">
                                    @livewire('tickets.details-tck', ['ticketID' => $tck->id], key('cerrado' . $tck->id))
                                    <template x-if="menu">
                                        <span>Ver más</span>
                                    </template>
                                </div>
