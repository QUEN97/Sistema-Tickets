<div>
    <div class="py-4 space-y-4">
        ...
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            {{-- Barra de Busqueda --}}
            <div>
                <x-input wire:model="search" type="text" class="w-auto" placeholder="Buscar zonas..." />
            </div>
            {{-- Acciones Masivas --}}
            @if ($checked)
                <x-dropdown align="right" width="60">
                    ...
                    <x-slot name="content">
                        <div class="w-60">
                            ...
                            <div>
                                @if ($valid->pivot->de == 1)
                                    <x-dropdown-link href="#" wire:click="deleteZonas">
                                        {{ __('Eliminar Zona') }}
                                    </x-dropdown-link>
                                @endif
        ...
        @if ($selectPage)
            @if ($selectAll)
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ $zonas->total() }}</strong> elementos.
                </div>
            @else
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ count($checked) }}</strong> elementos, ¿Deseas seleccionar los
                    <strong>{{ $zonas->total() }}</strong>?
                ...
                <x-slot name="body">
                    @forelse($zonas as $zona)
                        ...
                        <x-row wire:loading.class.delay="opacity-75">
                            ...
                            <x-cell class="font-bold">{{ $zona->name }}</x-cell>
                            <x-cell>
                                <span
                            ...
                            <x-cell> {{ $zona->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                            </x-cell>
                            <x-cell>
            ...
            @forelse($zonas as $zona)
                <div class="bg-white dark:bg-slate-900 space-y-3 p-4 rounded-lg shadow">
                            ...
                            <div class="relative inline-block text-left">
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    <div class="flex gap-2 justify-center items-center">
                                        <div>
                                            @if ($valid->pivot->ed == 1)
                                                @livewire('zonas.zona-edit', ['zona_id' => $zona->id], key('edit' . $zona->id))
                                            @endif
                                        </div>
                                        <div>
                                            @if ($valid->pivot->vermas == 1)
                                                @livewire('zonas.show-zona', ['zona_show_id' => $zona->id], key('showzona' . $zona->id))
                                            @endif
                                        </div>
                                        <div>
                                            @if ($valid->pivot->de == 1)
                                                @livewire('zonas.zona-delete', ['zonaID' => $zona->id], key('delete' . $zona->id))
                                            @endif
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="flex items-center space-x-2 text-sm">
                        <div class="font-bold">#{{ $zona->id }}</div>
                    ...
                    <div class="flex gap-4 justify-center items-center md:flex-row md:items-center md:justify-center ">
                        ...
                        <div>
                            {{ $zona->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                        </div>
                    </div>
                </div>
            @empty
