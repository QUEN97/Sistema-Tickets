<div>
    <button wire:click="ConfirmDelete({{$solicID}})" wire:loading.attr="disabled" class="tooltip">
    ...
    <x-dialog-modal wire:model="modalDelete" class="flex items-center">
        ...
        <x-slot name="content" class="relative">
            <div>
                ...
                <span class="dark:text-white">¿Está seguro de eliminar la solicitud: <i>"{{$tName}}"</i> de la estación: <i>{{$eName}}</i> ?</span>
            </div>
        </x-slot>
        <x-slot name="footer" >
            <div class="w-full flex justify-center gap-3">
                <x-danger-button class="mr-2" wire:click="DeleteSolicitud({{$solicID}})" wire:loading.attr="disabled">
                    Aceptar
                </x-danger-button>
