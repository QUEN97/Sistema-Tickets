<div>
    <button wire:click="showPrioridad({{$prioridadID}})" wire:loading.attr="disabled" class="tooltip">
    ...
    <x-dialog-modal wire:model="modal" class="flex items-center">
        <x-slot name="title">
            {{ __('Información de de la prioridad') }}
        </x-slot>
        ...
        @if ($prioridad)
            <div class=" flex justify-evenly pb-4">
                    <div>
                        ...
                        <div class="text-lg">{{$prioridad->name}}</div>
                    </div>
                    <div>
                        <div>
                            <h2>Cantidad de fallas:</h2>
                            <div class="text-lg">{{$prioridad->fallas->count()}}</div>
                        </div>
                    </div>
                
            </div>
            @if ($prioridad->fallas->count() > 0)
                    <div class="border rounded-lg  max-h-[320px] overflow-auto dark:border-gray-700">
                        <details>
                            <summary class="bg-gray-100 py-2 px-4 cursor-pointer dark:bg-slate-700 dark:text-gray-300 dark:border-gray-700">Click para mostrar/ocultar fallas asociadas</summary>
                            <table class="w-full">
                                <thead>
                                    <tr>
                ...
                <div class="flex flex-col justify-center items-center gap-3 py-6 text-gray-400">
                    ...
                    <span class="text-lg">Sin fallas registradas</span>
                </div>
                @endif
