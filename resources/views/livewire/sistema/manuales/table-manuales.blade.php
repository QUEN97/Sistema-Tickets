                ...
                @foreach ($manuals as $tem)
                    @foreach ($tem->permisos as $item)
                        @if ($item->pivot->permiso_id == Auth::user()->permiso_id)
                            <div class="relative">
                                <!-- Checkbox -->
                                <x-input type="checkbox" value="{{ $tem->id }}" wire:model="checked" class="absolute top-0 left-0 mt-2 ml-2 z-10"/>
                            
                                <!-- Contenedor principal -->
                                <div class="mx-auto mt-2 w-full transform overflow-hidden rounded-lg bg-white dark:bg-dark-eval-1 shadow-md hover:shadow-lg">
                                    <!-- Resto del contenido -->
