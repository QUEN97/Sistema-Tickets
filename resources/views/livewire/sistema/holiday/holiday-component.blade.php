<div class="w-full">
    <div class="w-full">
        <div class="text-sm text-gray-600 dark:text-gray-400 text-center mx-auto">
                    ...
                    @forelse ($holidays as $holy)
                        <tr>
                            <td
                                ...
                                <span
                                    class="lg:hidden top-0 left-0 bg-gray-300 px-1 py-1 text-xs font-bold uppercase dark:text-black">Nombre</span>
                                {{ $holy->name }}
                            </td>
                            <td
                                class="w-full lg:w-auto p-3 text-gray-800  dark:text-white border border-b block lg:table-cell relative lg:static dark:border-gray-700">
                                <span
                                    class="lg:hidden top-0 left-0 bg-gray-300 px-1 py-1 text-xs font-bold uppercase dark:text-black">Fecha</span>
                                {{ \Carbon\Carbon::parse($holy->date)->format('d-m-Y') }}
                            </td>
                            <td
                                class="w-full lg:w-auto p-3 text-gray-800  dark:text-white border border-b block lg:table-cell relative lg:static dark:border-gray-700">
                                <span
                                    class="lg:hidden top-0 left-0 bg-gray-300 px-1 py-1 text-xs font-bold uppercase dark:text-black">opciones</span>
                                @if ($valid->pivot->de == 1)
        ...
        <div class="mt-5">
            @if ($valid->pivot->wr == 1)
                <x-button wire:click="openModal" wire:loading.attr="disabled">
                    {{ __('Crear Festivo/Inhábil') }}
                </x-button>
            @endif
        </div>


        <x-dialog-modal wire:model="ModalFestivo" class="flex items-center">
            <x-slot name="title">
                <div class="bg-dark-eval-1 p-4 rounded-md text-white text-center">
                    {{ __('Nuevo Festivo/Inhábil') }}
                </div>
            </x-slot>

            <x-slot name="content">
                {{ __('Antes de proceder con la creación del festivo/inhábil, favor de validar si este ya se encuentra registrado, de lo contrario puede proceder con la creación de la fecha.') }}
                ...
                <div class="flex flex-col md:flex-row gap-2 mt-4">
                    ...
                    <div class="md:w-1/2 px-3 mb-6 md:mb-2">
                        ...
                        <x-input wire:model.defer="date"
                            class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm1 dark:border-gray-600 dark:bg-dark-eval-1
                        dark:text-gray-300 dark:focus:ring-offset-dark-eval-1{{ $errors->has('date') ? 'is-invalid' : '' }}"
            ...
            <x-slot name="footer">
                <x-button variant="danger" wire:click="$toggle('ModalFestivo')" wire:loading.attr="disabled">
                    {{ __('Cancelar') }}
                </x-button>

                @if ($valid->pivot->wr == 1)
                    <x-button class="ml-2" wire:click="createHoliday" wire:loading.attr="disabled">
                        <div role="status" wire:loading wire:target="createHoliday">
