<div>
    <div class="py-4 space-y-4">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            {{-- Barra de Busqueda --}}
            <div>
                <x-input wire:model="search" type="text" class="w-auto" placeholder="Buscar departamentos..." />
            </div>
            {{-- Acciones Masivas --}}
            @if ($checked)
                <x-dropdown align="right" width="60">
                    ...
                    <x-slot name="content">
                        <div class="w-60">
                            ...
                            <div>
                                @if ($valid->pivot->de == 1)
                                    <x-dropdown-link href="#" wire:click="deleteDeptos">
        ...
        @if ($selectPage)
            @if ($selectAll)
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ $departamentos->total() }}</strong> elementos.
                </div>
            @else
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ count($checked) }}</strong> elementos, ¿Deseas seleccionar los
                    <strong>{{ $departamentos->total() }}</strong>?
        ...
        <div class="overflow-auto rounded-lg shadow hidden md:block">
            ...
            <x-table>
                <x-slot name="head">
                    ...
                    <x-heading sortable wire:click="sortBy('name')" :direction="$sortField === 'name' ? $sortDirection : null">DEPARTAMENTO</x-heading>
                ...
                <x-slot name="body">
                    @forelse($departamentos as $depto)
                        ...
                        <x-row wire:loading.class.delay="opacity-75">
                            {{-- Componente Column --}}
                            <x-cell> <x-input type="checkbox" value="{{ $depto->id }}" wire:model="checked" />
                            </x-cell>
                            <x-cell>{{ $depto->id }} </x-cell>
                            <x-cell class="font-bold">{{ $depto->name }}</x-cell>
                            <x-cell>
                                <span
                                    class="rounded bg-{{ $depto->status_color }}-200 py-1 px-3 text-xs text-{{ $depto->status_color }}-500 font-bold">
                                    {{ $depto->status }}
                                </span>
                            </x-cell>
                            <x-cell> {{ $depto->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                            </x-cell>
                            <x-cell>
                                <div class="flex gap-2 justify-center items-center">
                                    @if ($valid->pivot->vermas == 1)
                                    <div>
                                         @livewire('departamentos.show-depto', ['deptoID' => $depto->id], key('show'.$depto->id))
                                    </div>
                                    @endif
                                    @if ($valid->pivot->ed == 1)
                                    <div>
                                        @livewire('departamentos.edit-depto', ['deptoID' => $depto->id], key('ed'.$depto->id))
                                    </div>
                                    @endif
                                    @if ($valid->pivot->de == 1)
                                    <div>
                                        @livewire('departamentos.delete-depto', ['deptoID' => $depto->id], key('del'.$depto->id))
                                    </div>
                                    @endif
                                </div>
                            </x-cell>
            ...
            @forelse($departamentos as $depto)
                <div class="bg-white dark:bg-slate-900 space-y-3 p-4 rounded-lg shadow">
                            ...
                            <div class="relative inline-block text-left">
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    <div class="flex gap-2 justify-center items-center">
                                        @if ($valid->pivot->vermas == 1)
                                        <div>
                                             @livewire('departamentos.show-depto', ['deptoID' => $depto->id], key('showdepto'.$depto->id))
                                        </div>
                                        @endif
                                        @if ($valid->pivot->ed == 1)
                                        <div>
                                            @livewire('departamentos.edit-depto', ['deptoID' => $depto->id], key('edit'.$depto->id))
                                        </div>
                                        @endif
                                        @if ($valid->pivot->de == 1)
                                        <div>
                                            @livewire('departamentos.delete-depto', ['deptoID' => $depto->id], key('delete'.$depto->id))
                                        </div>
                                        @endif
                                    </div>
                                </div>
                    ...
                    <div class="flex items-center space-x-2 text-sm">
                        <div class="font-bold">#{{ $depto->id }}</div>
                    ...
                    <div class="flex gap-4 justify-center items-center md:flex-row md:items-center md:justify-center ">
                        ...
                        <div>
                            {{ $depto->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                        </div>
                    </div>
                </div>
            @empty
