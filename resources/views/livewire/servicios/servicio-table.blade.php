                ...
                <x-dropdown align="right" width="60">
                    ...
                    <x-slot name="content">
                        <div class="w-60">
                            ...
                            <div>
                                @if ($valid->pivot->de == 1)
                                    <x-dropdown-link href="#" wire:click="deleteServicios">
        ...
        @if ($selectPage)
            @if ($selectAll)
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ $servicios->total() }}</strong> elementos.
                </div>
            @else
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ count($checked) }}</strong> elementos, ¿Deseas seleccionar los
                    <strong>{{ $servicios->total() }}</strong>?
        ...
        <div class="overflow-auto rounded-lg shadow hidden md:block">
            ...
            <x-table>
                <x-slot name="head">
                    ...
                    <x-heading sortable wire:click="sortBy('area_id')" :direction="$sortField === 'area_id' ? $sortDirection : null">ÁREA DESIGNADA</x-heading>
                ...
                <x-slot name="body">
                    @forelse($servicios as $servicio)
                        ...
                        <x-row wire:loading.class.delay="opacity-75">
                            {{-- Componente Column --}}
                            <x-cell> <x-input type="checkbox" value="{{ $servicio->id }}" wire:model="checked" />
                            </x-cell>
                            <x-cell>{{ $servicio->id }} </x-cell>
                            <x-cell class="font-bold">{{ $servicio->name }}</x-cell>
                            <x-cell> {{ $servicio->area->name }}</x-cell>
                            <x-cell>
                                <span
                                    class="rounded bg-{{ $servicio->status_color }}-200 py-1 px-3 text-xs text-{{ $servicio->status_color }}-500 font-bold">
                                    {{ $servicio->status }}
                                </span>
                            </x-cell>
                            <x-cell> {{ $servicio->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                            </x-cell>
                            <x-cell>
                                <div class="flex gap-2 justify-center items-center">
                                    @if ($valid->pivot->vermas == 1)
                                        @livewire('servicios.show-servicio', ['servicioID' => $servicio->id],key('show'.$servicio->id))
                                    @endif
                                    @if ($valid->pivot->ed == 1)
                                        @livewire('servicios.edit-servicio', ['servicioID' => $servicio->id],key('ed'.$servicio->id))
                                    @endif
                                    @if ($valid->pivot->de == 1)
                                        @livewire('servicios.delete-servicio', ['servicioID' => $servicio->id],key('del'.$servicio->id))
                                    @endif
                                </div>
                            </x-cell>
                        </x-row>
            ...
            @forelse($servicios as $servicio)
                <div class="bg-white dark:bg-slate-900 space-y-3 p-4 rounded-lg shadow">
                        ...
                        <div x-data="{ open: false }">
                            <div class="relative inline-block text-left">
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    <div class="flex gap-2 justify-center items-center">
                                        @if ($valid->pivot->vermas == 1)
                                        @livewire('servicios.show-servicio', ['servicioID' => $servicio->id],key('showserv'.$servicio->id))
                                    @endif
                                    @if ($valid->pivot->ed == 1)
                                        @livewire('servicios.edit-servicio', ['servicioID' => $servicio->id],key('edit'.$servicio->id))
                                    @endif
                                    @if ($valid->pivot->de == 1)
                                        @livewire('servicios.delete-servicio', ['servicioID' => $servicio->id],key('delete'.$servicio->id))
                                    @endif
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="flex items-center space-x-2 text-sm">
                        <div class="font-bold">#{{ $servicio->id }}</div>
                    ...
                    <div class="flex gap-4 justify-center items-center md:flex-row md:items-center md:justify-center ">
                        ...
                        <div>
                            {{ $servicio->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                        </div>
                    </div>
                </div>
            @empty
