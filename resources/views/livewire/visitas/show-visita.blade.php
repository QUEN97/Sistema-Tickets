<div>
    <button wire:click="showVisita({{ $visitaID }})" wire:loading.attr="disabled" class="text-gray-400 duration-300 flex items-center gap-2 hover:text-gray-600">
    ...
    <x-dialog-modal wire:model="modal" class="flex items-center">
        ...
        <x-slot name="content" class="relative">
                ...
                <div>
                    @foreach ($visita->usuario as $user)
                        @if ($user->profile_photo_path)
                            <div onclick="window.location.href='{{ asset('/storage/' . $user->profile_photo_path) }}'">
                        ...
                        @else
                            <div onclick="window.location.href='{{ asset($user->profile_photo_url) }}'">
                                <img class="relative inline-block h-[58px] w-[58px] rounded-full object-cover object-center"
                                    src="{{ $user->profile_photo_url }}" alt="{{ $user->profile_photo_url }}" />
                            </div>
                        @endif
                        ...
                        <div class="flex items-center mt-3 mb-3"> <!-- Agregado el contenedor flex -->
                            @if ($evidenciaArc)
                                <div class="relative" x-data="{ toggle: false }">
                                    <button class="text-gray-400 duration-300 flex items-center gap-2 hover:text-gray-600"
                                        @click="toggle=!toggle">
                                    ...
                                    @foreach ($evidenciaArc as $antigArch)
                                        @if ($antigArch->flag_trash == 0)
                                            <div class="absolute z-50 flex flex-col w-max rounded-md overflow-hidden bg-white dark:bg-dark-eval-3 shadow-md right-full top-0"
            ...
            <div class="p-0 mb-6">
                ...
                <p class="block font-sans text-base antialiased font-light leading-relaxed text-inherit max-h-[120px] overflow-y-auto">
                    "{{ $visita->observacion_visita }}"
                </p>
            </div>
        </x-slot>
