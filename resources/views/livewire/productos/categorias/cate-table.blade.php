<div>
    <div class="py-4 space-y-4">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            {{-- Barra de Busqueda --}}
            <div>
                <x-input wire:model="search" type="text" class="w-auto" placeholder="Buscar categorías..." />
            </div>
            {{-- Acciones Masivas --}}
            @if ($checked)
                <x-dropdown align="right" width="60">
                    ...
                    <x-slot name="content">
                        <div class="w-60">
                            ...
                            <div>
                                @if ($valid->pivot->de == 1)
                                    <x-dropdown-link href="#" wire:click="deleteCategorias">
        ...
        @if ($selectPage)
            @if ($selectAll)
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ $categorias->total() }}</strong> elementos.
                </div>
            @else
                <div class="text-gray-400 text-xs">
        ...
        <div class="overflow-auto rounded-lg shadow hidden md:block">
            ...
            <x-table>
                <x-slot name="head">
                    ...
                    <x-heading sortable wire:click="sortBy('name')" :direction="$sortField === 'name' ? $sortDirection : null">CATEGORÍA</x-heading>
                ...
                <x-slot name="body">
                    @forelse($categorias as $categoria)
                        ...
                        <x-row wire:loading.class.delay="opacity-75">
                            {{-- Componente Column --}}
                            <x-cell> <x-input type="checkbox" value="{{ $categoria->id }}" wire:model="checked" />
                            </x-cell>
                            <x-cell>{{ $categoria->id }} </x-cell>
                            <x-cell class="font-bold">{{ $categoria->name }}</x-cell>
                            <x-cell>
                                <span
                                    class="rounded bg-{{ $categoria->status_color }}-200 py-1 px-3 text-xs text-{{ $categoria->status_color }}-500 font-bold">
                                    {{ $categoria->status }}
                                </span>
                            </x-cell>
                            <x-cell> {{ $categoria->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                            </x-cell>
                            <x-cell>
                                <div class="flex gap-2 justify-center items-center">
                                    <div>
                                        @if ($valid->pivot->vermas == 1)
                                            @livewire('productos.categorias.categoria-show', ['categoria_show_id' => $categoria->id], key('show'.$categoria->id))
                                        @endif
                                    </div>
                                    <div>
                                        @if ($valid->pivot->ed == 1)
                                            @livewire('productos.categorias.categoria-edit', ['categoria_id' => $categoria->id], key('ed'.$categoria->id))
                                        @endif
                                    </div>
                                    <div>
                                        @if ($valid->pivot->de == 1)
                                            @livewire('productos.categorias.categoria-delete', ['catID' => $categoria->id], key('del'.$categoria->id))
                                        @endif
                                    </div>
                                </div>
                            </x-cell>
            ...
            @forelse($categorias as $categoria)
                <div class="bg-white dark:bg-slate-900 space-y-3 p-4 rounded-lg shadow">
                            ...
                            <div class="relative inline-block text-left">
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    <div class="flex gap-2 justify-center items-center">
                                        <div>
                                            @if ($valid->pivot->vermas == 1)
                                                @livewire('productos.categorias.categoria-show', ['categoria_show_id' => $categoria->id], key('showcate'.$categoria->id))
                                            @endif
                                        </div>
                                        <div>
                                            @if ($valid->pivot->ed == 1)
                                                @livewire('productos.categorias.categoria-edit', ['categoria_id' => $categoria->id], key('edit'.$categoria->id))
                                            @endif
                                        </div>
                                        <div>
                                            @if ($valid->pivot->de == 1)
                                                @livewire('productos.categorias.categoria-delete', ['catID' => $categoria->id], key('delete'.$categoria->id))
                                            @endif
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="flex gap-4 justify-center items-center md:flex-row md:items-center md:justify-center ">
                        ...
                        <div>
                            {{ $categoria->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                        </div>
                    </div>
                </div>
            @empty
