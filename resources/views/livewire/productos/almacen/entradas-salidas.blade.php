<div x-data="{ modelOpen: false }">
    <button @click="modelOpen =!modelOpen" wire:loading.attr="disabled" aria-label="añadir tipo" class="flex gap-2 items-center px-4 py-2 rounded-md bg-black text-white font-semibold hover:bg-gray-700 ">
    ...
    <div x-show="modelOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 text-center md:items-center sm:p-0">
            ...
            <div x-cloak x-show="modelOpen" 
                ...
                <div class="my-2" x-data="{ productos:prod,estaciones:est,tickets:tck,contador:1,
                    ...
                    <div class=" text-center text-base">
                        <div class="">
                            ...
                            <div class="flex gap-2 justify-center py-3">
                                <div id="tipo">
                                    <input type="radio" name="tipo" id="entrada" value="entrada" wire:model.defer="tipo" class="peer/entrada hidden">
                                    <label for="entrada" class="flex items-center justify-center gap-1 cursor-pointer bg-gray-300 dark:bg-dark-eval-0 peer-checked/entrada:bg-black hover:bg-black text-white px-4 py-2 rounded-md">
                                ...
                                <div>
                                    <input type="radio" name="tipo" id="salida" value="salida" wire:model.defer="tipo" class="peer/salida hidden">
                                    <label for="salida" class="flex items-center justify-center gap-1 cursor-pointer bg-gray-300 dark:bg-dark-eval-0 peer-checked/salida:bg-black hover:bg-black text-white px-4 py-2 rounded-md">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                            <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                            <path d="M12 21l-8 -4.5v-9l8 -4.5l8 4.5v4.5"></path>
                                            <path d="M12 12l8 -4.5"></path>
                    ...
                    <div>
                        <div class="relative border-b border-gray-400 pb-2">
                            <input type="text" name="motivo" id="motivo" required autofocus autocomplete="base" wire:model.defer="motivo"
                            placeholder=" "  class="peer w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm1 dark:border-gray-600 dark:bg-dark-eval-1 dark:text-gray-300 dark:focus:ring-offset-dark-eval-1">
                            <label for="motivo" class="absolute rounded-md duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-dark-eval-1 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1"> {{ __('Motivo') }}</label>
                        </div>
                    ...
                    {{-- <div class="py-2">
                        ...
                        <div class="mt-2 p-2 flex flex-wrap gap-2 max-h-80 overflow-auto" >
                            <template x-for="prod in lista" :key="prod.id">
                                <div x-data="{options:false}" class="w-full">
                                    <div class="overflow-hidden">
                                        <input type="checkbox" name="productos[]"  class="peer hidden" :id="`p${prod.id}`">
                                        <label class="border flex flex-col gap-2 items-center w-full cursor-pointer p-2 rounded-md transition duration-300 delay-100 overflow-hidden max-[400px]:flex-wrap" :for="`p${prod.id}`"  :class="prod.show?'bg-blue-500 border-blue-500 dark:bg-indigo-700 dark:border-indigo-700':'border-gray-300 dark:border-gray-500'">
                                            <div class="flex flex-wrap justify-start gap-2 items-center w-full" @click="prod.show=!prod.show">
                                                <div class=" flex gap-2 items-center flex-col min-[380px]:flex-row" >
                                                    <figure class="max-w-[3rem] max-h-[3rem] rounded-full overflow-hidden">
                                                        <img  class="w-full object-cover" alt="imagen producto" :src="`/storage/${prod.archivo_path}`">
                                                    </figure>
                                                    <div x-text="prod.name" class="transition duration-300 delay-100" :class="prod.show?'text-white':''"></div>
                                                </div>
                                                <div class="text-center text-white flex flex-wrap gap-2 border-l-2 px-2 border-white" x-cloak x-show="prod.show" x-transition>
                                                    En stock: 
                                                    <p x-text="prod.cant"></p>
                                                </div>
                                            </div>
                                            <div class="w-full flex flex-wrap gap-2" x-cloak x-show="prod.show" x-collapse>
                                                <div>
                                                ...
                                                <div>
                                                    <div class="relative">
                                                        ...
                                                        <label :for="`base${prod.id}`" class="absolute rounded-md duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-dark-eval-1 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1"> {{ __('Cantidad') }}</label>
                                                    </div>
                                                    <x-input-error for=""></x-input-error>
                                                </div>
                                                <div class="w-full relative mt">
                                                    <textarea class="w-full h-24 resize-none border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm1 dark:border-gray-600 dark:bg-dark-eval-1 dark:text-gray-300 dark:focus:ring-offset-dark-eval-1" :name="`area${prod.id}`" :id="`area${prod.id}`" cols="30" rows="10" x-model="prod.observacion"></textarea>
                                                    <label :for="`base${prod.id}`" class="absolute rounded-md duration-300 transform  z-10 origin-[0] bg-white dark:bg-dark-eval-1 px-2 text-blue-600 dark:text-blue-500  top-2 scale-75 -translate-y-4 left-1"> {{ __('Observación') }}</label>
                                                </div>
                                            </div>
                                        </label>
                                    </div>
                        ...
                        <template x-for="prod in carrito" :key="prod.id">
                            <div class=" py-3  border-b border-gray-400 relative">
                                <template x-if="prod.id>1">
                                    <button type="button" @click="remove(prod.id)" class="absolute top-3 right-0 text-white p-2 rounded-md bg-red-600 hover:bg-red-700 transition duration-300">
