        ...
        <div class="flex items-center justify-center min-h-screen px-4 text-center md:items-center sm:p-0">
            ...
            <div x-cloak x-show="modelOpen" 
                ...
                <div class="flex items-center justify-between space-x-4">
                    <h1 class="text-xl font-medium">{{ __('Editar producto del almacén') }}</h1>
                </div>
                <div>
                    <div>
                        ...
                        <div class="flex flex-wrap gap-3 justify-evenly items-center">
                            <div>
                                <x-label value="{{ __('Stock actual') }}" for="stock" />
                                <x-input wire:model.defer="stock" type="number" name="stock"
                                    id="stock" required autofocus autocomplete="stock" min=0 />
                                <x-input-error for="stock"></x-input-error>
                            </div>
                            <div>
                                <x-label value="{{ __('Stock base') }}" for="base" />
                                <x-input wire:model.defer="base" type="number" name="base"
                                    id="base" required autofocus autocomplete="base" min=0 />
                                <x-input-error for="base"></x-input-error>
                            </div>
                        </div>
                    </div>
                    <div name="footer" class="d-none text-right mt-3">
                        <x-danger-button class="mr-2" wire:click="updateAlmacen({{$idAlma}})"  wire:loading.attr="disabled">
                            <div role="status" wire:loading wire:target="updateAlmacen">
