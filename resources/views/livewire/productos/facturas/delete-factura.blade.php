<div>
    <button wire:click="$set('modalDelete',true)" wire:loading.attr="disabled" class="tooltip">
    ...
    <x-dialog-modal wire:model="modalDelete" class="flex items-center">
        ...
        <x-slot name="content" class="relative">
            <div>
                ...
                <span>¿Está seguro de eliminar el registro <i>#{{$facturaID}}</i> ?</span>
            </div>
        </x-slot>
        <x-slot name="footer" >
            <div class="w-full flex justify-center gap-3">
                <x-danger-button class="mr-2" wire:click="DeleteFactura({{$facturaID}})" wire:loading.attr="disabled">
                    Aceptar
                </x-danger-button>
