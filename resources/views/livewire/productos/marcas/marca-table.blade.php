<div>
    <div class="py-4 space-y-4">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            {{-- Barra de Busqueda --}}
            <div>
                <x-input wire:model="search" type="text" class="w-auto" placeholder="Buscar marcas..." />
            </div>
            {{-- Acciones Masivas --}}
            @if ($checked)
                <x-dropdown align="right" width="60">
                    ...
                    <x-slot name="content">
                        <div class="w-60">
                            ...
                            <div>
                                @if ($valid->pivot->de == 1)
                                    <x-dropdown-link href="#" wire:click="deleteMarcas">
                                        {{ __('Eliminar Marca') }}
                                    </x-dropdown-link>
                                @endif
        ...
        @if ($selectPage)
            @if ($selectAll)
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ $marcas->total() }}</strong> elementos.
                </div>
            @else
                <div class="text-gray-400 text-xs">
        ...
        <div class="overflow-auto rounded-lg shadow hidden md:block">
            ...
            <x-table>
                <x-slot name="head">
                    ...
                    <x-heading sortable wire:click="sortBy('name')" :direction="$sortField === 'name' ? $sortDirection : null">MARCA</x-heading>
                ...
                <x-slot name="body">
                    @forelse($marcas as $marca)
                        ...
                        <x-row wire:loading.class.delay="opacity-75">
                            {{-- Componente Column --}}
                            <x-cell> <x-input type="checkbox" value="{{ $marca->id }}" wire:model="checked" />
                            </x-cell>
                            <x-cell>{{ $marca->id }} </x-cell>
                            <x-cell class="font-bold">{{ $marca->name }}</x-cell>
                            <x-cell>
                                <span
                                    class="rounded bg-{{ $marca->status_color }}-200 py-1 px-3 text-xs text-{{ $marca->status_color }}-500 font-bold">
                                    {{ $marca->status }}
                                </span>
                            </x-cell>
                            <x-cell> {{ $marca->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                            </x-cell>
                            <x-cell>
                                <div class="flex gap-2 justify-center items-center">
                                    @if ($valid->pivot->vermas == 1)
                                    @livewire('productos.marcas.show-marca', ['marcaID' => $marca->id], key('show'.$marca->id))
                                @endif
                                @if ($valid->pivot->ed == 1)
                                    @livewire('productos.marcas.edit-marca', ['marcaID' => $marca->id], key('ed'.$marca->id))
                                @endif
                                @if ($valid->pivot->de == 1)
                                    @livewire('productos.marcas.delete-marca', ['marcaID' => $marca->id], key('del'.$marca->id))
                                @endif
                                </div>
                            </x-cell>
                        </x-row>
            ...
            @forelse($marcas as $marca)
                <div class="bg-white dark:bg-slate-900 space-y-3 p-4 rounded-lg shadow">
                        ...
                        <div x-data="{ open: false }">
                            <div class="relative inline-block text-left">
                                ...
                                <div x-show="open" @click.away="open = false"
                                    ...
                                    <div class="flex gap-2 justify-center items-center">
                                        @if ($valid->pivot->vermas == 1)
                                        @livewire('productos.marcas.show-marca', ['marcaID' => $marca->id], key('showmarca'.$marca->id))
                                    @endif
                                    @if ($valid->pivot->ed == 1)
                                        @livewire('productos.marcas.edit-marca', ['marcaID' => $marca->id], key('edit'.$marca->id))
                                    @endif
                                    @if ($valid->pivot->de == 1)
                                        @livewire('productos.marcas.delete-marca', ['marcaID' => $marca->id], key('delete'.$marca->id))
                                    @endif
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="flex items-center space-x-2 text-sm">
                        <div class="font-bold">#{{ $marca->id }}</div>
