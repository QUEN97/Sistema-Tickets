<div>
    <button wire:click="editRegion({{$regionID}})" wire:loading.attr="disabled" class="tooltip">
    ...
    <x-dialog-modal wire:model="modal" id="modalRepuesto" class="flex items-center">
        ...
        <x-slot name="content">
            <div class="flex flex-wrap gap-3 justify-evenly items-center">
                <div>
                    <x-label value="{{ __('Nombre de la región') }}" for="name" />
        ...
        <x-slot name="footer" class="d-none">
            <x-danger-button class="mr-2" wire:click="updateRegion({{$regionID}})" wire:loading.attr="disabled">
                Aceptar
            </x-danger-button>
